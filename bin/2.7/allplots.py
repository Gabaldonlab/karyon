...
def scaffold_len_lin(fasta):
	...
	plt.savefig(args.output+'_lenlin.png')
	plt.close()

def scaffold_len_log(fasta):
	lenlist = []
	fastainput = SeqIO.index(fasta, "fasta")
	for i in fastainput:
		lenlist.append(len(fastainput.get_raw(i).decode()))
	lenlist.sort()
	N = len(lenlist)
	...
	plt.axhline(y=window_size, color='dimgrey', linestyle='--', linewidth=2)
	plt.ylabel('Scaffold length')
	plt.savefig(args.output+'_lenlog.png')
	plt.close()

def extract_vcf_data (vcf_file, quality_filter):
	snp_count = 0
	curr_pos = 0
	curr_scaffold = ''
	snp_dict = {}
...
def var_v_cov(vcf, pileup):
	...
	plt.savefig(args.output+'_var_v_cov.'+'ws'+str(window_size)+'.png')
...
def cov_plot(pileup):
	...
	plt.savefig(args.output+'_covplot.'+'ws'+str(window_size)+'.png')
	plt.close()
	pileup_file.close()

def fair_coin_global(vcf):
	vcf_file = open(vcf)
	value_list = []
	binomial_list = []
	...
	plt.savefig(args.output+'_faircoin_global.'+'ws'+str(window_size)+'.png')
	vcf_file.seek(0)
	plt.close()

def fair_coin_scaff(vcf):
	vcf_file = open(vcf)
	scaffold_number = args.counter
	value_dict, alt_dict = {}, {}
	size_list = []
	...
	plt.xlabel('Alternative/Reference SNP')
	plt.ylabel('Frequency')
	plt.savefig(args.output+'_faircoin_by_scaff.'+'ws'+str(window_size)+'.png')
	vcf_file.seek(0)
	plt.close()

def cov_v_len(pileup, fasta):
	fastainput = SeqIO.index(fasta, "fasta")
	pileup_file = open(pileup)
	x = []
	y = []
	...
	plt.savefig(args.output+'_len_v_cov.png')
	pileup_file.seek(0)
...
def katplot(fasta, library):
	os.system(KAT+" comp -o "+args.output+" "+args.library+" "+args.fasta+" > "+args.output+".katreport")
...
#window_walker(window_size, step, vcf_file, fasta_file, bam_file, nQuire, kitchen)
scaffold_len_lin(args.fasta)
scaffold_len_log(args.fasta)
var_v_cov(args.vcf, args.pileup)
cov_plot(args.pileup)
fair_coin_global(args.vcf)
fair_coin_scaff(args.vcf)
cov_v_len(args.pileup, args.fasta)


katplot(args.fasta, args.library)
