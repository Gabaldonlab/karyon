...
def var_call(fastq, config_dict, output, name, favourite, home, memory, nodes, reduced_assembly, no_red_assembly, no_reduction):
	...
	bash_job.write(config_dict["bcftools"][0]+"bcftools mpileup --threads "+nodes+" --no-reference " + locspp+'.sorted.bam --output-type v > '+locspp+'.mpileup\n')
	bash_job.write(config_dict["samtools"][0]+"samtools flagstat -@ "+nodes+" "+locspp+'.sorted.bam > '+locspp+'.flagstat\n')
	...
	bash_job.write(config_dict["nQuire"][0]+" lrdmodel -t "+nodes+" "+ locspp+'.bin > '+locspp+'.lrdtest\n')
