...
def get_nQuire(nQuirefile):
    nQlist = []
    for line_bytes in open(nQuirefile, "r"):
        try:
            line = line_bytes.decode("utf-8", errors="replace").strip()
        except:
            line = line_bytes.strip()
        if line.split()[0] == "file" or len(line) == 0:
            continue
        else:
...
def get_flagstat(flagstat):
    mapped = 0
    prop_paired = 0
    for line_bytes in open(flagstat, "r"):
        try:
            line = line_bytes.decode("utf-8", errors="replace").strip()
        except:
            line = line_bytes.strip()
...
def report(
    ...
    if mybusco != False:
        ...
        if noredbusco != False and no_reduction != True:
            ...
            if buscoval[0] / noredbuscoval[0] < 0.9:
                report.write(
                    "WARNING: Reduction has caused loss of information, based on BUSCO values before and after reduction.\n"
                )
        if os.path.isdir(
            location + "no_reduc_busco"
        ) == True and name + "no_reduc.busco" not in os.listdir(true_output):
            report.write(
                "WARNING: Busco evaluation for the non-reduced assembly seems to have failed."
            )
    if os.path.isdir(location + "_busco") == True and name + ".busco" not in os.listdir(
        true_output
    ):
    ...
    if no_reduction != True:
        ...
        report.write(
            ...
            + str(fastats[1])
            + " ("
            + str(100 * (redustats[1] - fastats[1]) / redustats[1])
            + "%)\n"
        )
        report.write(
            "Assembly size: From "
        ...
        report.write(
            "Length of scaffolds > 1000bp:: From "
            + str(redustats[5])
            + " to "
            + str(fastats[5])
            + " ("
    ...
    report.write(
        "Diploid regions have an average of "
        + str(df.loc[df["ploidy"] == 2].SNPs.mean() / float(window_size))
        + "% of variant loci\n"
    )
    report.write("\n")
    ...
    for entry in fastadict:
        ...
        for i in range(1, 4):
            if len(
                contigdata.loc[contigdata["ploidy"] == i].count()
            ) / nwindows > 0.5 and i != b.index(max(b[1:])):
                report.write(
                    "Scaffold "
                    + fastadict[entry].id
                    + ", with a length of "
                    + str(len(fastadict[entry].seq))
                    + " base pairs, has a "
                    + str(
                        100
                        * (
                            len(contigdata.loc[contigdata["ploidy"] == i].count())
                            / nwindows
                        )
                    )
                    + " % of windows that are "
...
def ploidy_veredict(df, true_output, window_size):
    ...
    a = (
        b.loc[b.diplo_score > b.triplo_score]
        .loc[b.diplo_score > b.triplo_score]
        .loc[:, "mean_cov"]
    )
    empir_mean, empir_stdev = np.mean(a), np.std(a)
    hap_normal, tetra_normal = np.random.normal(
        empir_mean / 2, empir_stdev / 2, 10000
    ), np.random.normal(empir_mean * 2, empir_stdev * 2, 10000)
    ...
    for i in range(0, len(df)):
        if (
            df["mean_cov"][i] > hap25
            and df["mean_cov"][i] < hap75
            and (float(df["SNPs"][i])) / float(window_size) <= 0.005
        ):
            df.at[i, "ploidy"] = 1
        elif df["diplo_score"][i] != np.NAN:
            if (
                df["mean_cov"][i] > tetra25
                and df["mean_cov"][i] < tetra75
                and df["tetra_score"][i] > df["diplo_score"][i]
                and df["tetra_score"][i] > df["triplo_score"][i]
            ):
                df.at[i, "ploidy"] = 4
