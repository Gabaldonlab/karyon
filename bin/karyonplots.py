...
def extract_vcf_data(
    vcf_file, quality_filter, window_size, lendict, scafminsize, scafmaxsize
):
    snp_count = 0
    curr_pos = 0
    curr_scaffold = ""
    ...
    with open(vcf_file, 'r') as file:
        for line_bytes in file:
            ...
            if line[0] == "#":
                continue
            chunk = line.split("\t")
            if scafminsize != False and lendict[chunk[0]] <= scafminsize:
                continue
            elif scafmaxsize != False and lendict[chunk[0]] >= scafminsize:
                continue
            else:
                if float(chunk[5]) < quality_filter:
                    continue
                if chunk[0] != curr_scaffold:
                    ...
                    if (int(chunk[1]) - curr_pos) / window_size > 1:
                        for i in range(1, int((int(chunk[1]) - curr_pos) // window_size)):
                            snp_dict[
                                curr_scaffold + "_" + str(curr_pos + (i * window_size))
                            ] = 0
                    curr_pos = curr_pos + (
                        window_size * ((int(chunk[1]) - curr_pos) / window_size)
                    )
                    snp_count = 1
                else:
        ...
        for line_bytes in file:
            ...
            if scafminsize != False and lendict[chunk[0]] <= scafminsize:
                ...
                if chunk[0] != curr_scaffold:
                    ...
                    # Split the INFO field into individual key-value pairs
                    info_pairs = str(info_field).split(';')

                    # Extract the value of DP directly
                    dp_value = info_pairs[0].split('=')[1]

                    coverage.append(int(dp_value))
                else:
                    res = max(line, key=len)
                    info_field = chunk[7]
                    info_pairs = str(info_field).split(';')
...
def var_v_cov(vcf, pileup, window_size, output, lendict, scafminsize, scafmaxsize):
    ...
    pileup_file = pileup
    quality_filter = 500
    snp_density = extract_vcf_data(
        vcf_file, quality_filter, window_size, lendict, scafminsize, scafmaxsize
    )
    mean_cov = extract_pileup_data(
        pileup_file, window_size, lendict, scafminsize, scafmaxsize
    )
    x, y = [], []
    ...
    for element in snp_density:
        if (
            element in mean_cov
            and mean_cov[element] < 200
            and mean_cov[element] > 10
            and snp_density[element] < 100
        ):
...
def cov_plot(pileup, window_size, output, lendict, scafminsize, scafmaxsize):
    ...
    cov_dict = {}
    fig = plt.figure(figsize=(15, 10))
    for line_bytes in open(pileup_file, "r"):
        try:
            line = line_bytes.decode("utf-8", errors="replace").strip()
        except:
            line = line_bytes.strip()
        ...
        if scafminsize != False and lendict[chunk[0]] <= scafminsize:
            ...
            if chunk[0] != curr_scaffold:
                ...
                try:
                    ...
                    print("Could not append the coverage %s in from the pileup file. The line is the following one: \n")
                    print(line)
                    print("\nTill here")
    data = []
    meanlist = []
                        ...
                        if (int(values[0]) + int(values[1])) == 0:
                            ...
                            binomial_list.append(
                                numpy.random.binomial(
                                    n=(int(values[0]) + int(values[1])), p=0.5, size=None
                                )
                                / 1
                            )
                        else:
                            value_list.append(
                                float(values[1]) / (int(values[0]) + int(values[1]))
                            )
                            binomial_list.append(
                                numpy.random.binomial(
                                    n=(int(values[0]) + int(values[1])), p=0.5, size=None
...
def fair_coin_scaff(
    vcf, window_size, counter, output, lendict, scafminsize, scafmaxsize
):
    vcf_file = vcf
    ...
    with open(vcf_file, 'r') as file:
        for line_bytes in file:
            ...
            if scafminsize != False and lendict[chunk[0]] <= scafminsize:
                ...
                if chunk[0] not in value_dict:
                    if len(value_dict) >= scaffold_number:
                        break
                    else:
                        value_dict[chunk[0]] = []
                ...
                if chunk[-1].split(":")[0] == "0/1":
                    ...
                    if int(values[0]) + int(values[1]) >= cutoff:
                        value_dict[chunk[0]].append(
                            float(values[1]) / (int(values[0]) + int(values[1]))
                        )
                        alt_dict[chunk[0] + "_alt"].append(
                            float(values[0]) / (int(values[0]) + int(values[1]))
                        )
                        binomial_value = numpy.random.binomial(
                            n=(int(values[0]) + int(values[1])), p=0.5, size=None
                        ) / (float(values[0]) + int(values[1]))
        ...
        for e in value_dict:
            ...
            binomial_sublist = numpy.random.choice(
                binomial_list, size=sampling, replace=False
            )
...
def launch_nQuire(BAMtemp, nQuire, kitchen):
    ...
    os.system(
        nQuire + " create -b " + kitchen + "BAMtemp.bam -o " + kitchen + "nQuire_temp"
    )
    os.system(
        nQuire + " lrdmodel " + kitchen + "nQuire_temp.bin > " + kitchen + "nQuire_temp.report"
    )
    nQuire_report = kitchen + "nQuire_temp.report"

    free_score, diplo_score, triplo_score, tetra_score = 0.0, 0.0, 0.0, 0.0
    for line_bytes in open(nQuire_report, "r"):
        try:
            line = line_bytes.decode("utf-8", errors="replace").strip()
        except:
            line = line_bytes.strip()
    ...
    if free_score < 0.001:
        free_score, diplo_score, triplo_Score, tetra_Score = (
            float("nan"),
            float("nan"),
            float("nan"),
            float("nan"),
    ...
    return (
        round(diplo_score / free_score, 3),
        round(triplo_score / free_score, 3),
        round(tetra_score / free_score, 3),
        free_score,
    )
...
def ttest_ploidy(number_list):
    ...
    R2_triploidA, R2_triploidB = (
        stats.ttest_1samp(number_list, math.log(2.0, 2), nan_policy="omit")
    ), (stats.ttest_1samp(number_list, math.log(2.0, 2) * -1, nan_policy="omit"))
    R2_tetraploidA, R2_tetraploidB = (
        stats.ttest_1samp(number_list, math.log(3.0, 2), nan_policy="omit")
    ), (stats.ttest_1samp(number_list, math.log(1 / 3.0, 2), nan_policy="omit"))
...
def window_walker(
    ...
    for record in fasta:
        if len(os.listdir(newpath + "nQuireplots_ws" + str(window_size))) >= counter:
            break
        ...
        while start + step <= end:
            ...
            VCF = vcf_file.fetch(
                contig=record.name, start=start, stop=start + window_size
            )
            mean_refaltcov_list = []
            for i in VCF:
                ...
                A = bam_file.count_coverage(
                    record.name, snp_pos, snp_pos + 1, quality_threshold=0
                )
            ...
            BAMtemp = pysam.AlignmentFile(
                kitchen + "BAMtemp.bam", "wb", template=bam_file
            )
            for i in BAMfetch:
                BAMtemp.write(i)
            BAMtemp.close()
            pysam.index(kitchen + "BAMtemp.bam")
            mean_cov = numpy.nanmean(
                bam_file.count_coverage(
                    record.name, start, start + window_size, quality_threshold=0
                )
            )

            stdev_cov = numpy.nanstd(
                bam_file.count_coverage(record.name, start, start + window_size)
            )
            diplo_score, triplo_score, tetra_score, free_score = launch_nQuire(
                BAMtemp, nQuire, kitchen
            )
            (
                R2_diploid,
                R2_triploidA,
            ...
            df = pd.DataFrame(
                columns=[
                    ...
                    "tetra_score",
                    "free_score",
                    "normalized_diplo_score",
                    "normalized_triplo_score",
                    "normalized_tetra_score",
                    "mean_cov",
                    "st_dev_cov",
                    "SNPs",
                    "ploidy",
            ...
            dfrow = pd.DataFrame(
                {
                    ...
                    "diplo_score": diplo_score * free_score,
                    "triplo_score": triplo_score * free_score,
                    "tetra_score": tetra_score * free_score,
                    "free_score": free_score,
                    "normalized_diplo_score": diplo_score,
                    "normalized_triplo_score": triplo_score,
                    "normalized_tetra_score": tetra_score,
                    "mean_cov": mean_cov,
                    "st_dev_cov": stdev_cov,
                    "SNPs": len(mean_refaltcov_list),
                    "ploidy": 0,
                },
                index=[N],
            )
...
def nQuire_plot(
    value_list, window_size, newpath, xcov, ycov, lendict, scafminsize, scafmaxsize
):
    ...
    if len(x) > 0:
        if scafminsize != False and lendict[name] <= scafminsize:
            ...
            # Diploid score
            fig, (p0, p1, p2, p3, p4) = plt.subplots(
                nrows=5, sharex=True, figsize=(45, 15)
            )
            plt.title(name)
            plt.subplot(5, 1, 1)
            plt.xlim(0, x[-1])
            ...
            if len(xy[0]) > 3:
                ...
                plt.scatter(
                    pos_list, all_refalt_list, c=z, s=30, edgecolor=(0.4, 0.4, 0.4)
                )
            plt.savefig(
                newpath + "nQuireplots_ws" + str(window_size) + "/" + name + ".png"
            )
            print(
                newpath
                + "nQuireplots_ws"
...
def katplot(fasta, library, KAT, out):
    """Diego's modification for BSC runs:
    KAT tends to not end the process after completition, so uncomment it with care. May require a fix but to my knowledge this is a common issue from KAT

    It has been also modified such as KAT is executed from a KAT conda environment shared for everyone"""
    # Modified the env and the number of threads for Y1000H project, originally it was "-n kat_env", substituted for the prefix param. Also change the desired threads as required
    cmd = (
        "conda run --prefix /gpfs/projects/bsc40/project/pipelines/anaconda3/envs/kat_env kat comp -t 44  --output_prefix "
        + out
        + " "
        + library
        + " "
    ...
    # [WARNING]: Commented this lines as for the project Y1000H runs KAT execution was not necessary. If you require it for future runs, comment this line
    # try:
...
def allplots(
    ...
    if no_plot == False:
        ...
        fair_coin_scaff(
            vcf, window_size, counter, newpath, lendict, scafminsize, scafmaxsize
        )
        cov_v_len(mpileup, fastainput, newpath)
        # Kat plots are giving issues at MN4, run it manually
